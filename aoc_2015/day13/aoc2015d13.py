...
soln_file = script_path / "input.txt"  #  664 / 640
test_file = script_path / "test.txt"  # 330 / 286
...
def build_relationships(data):
    ...
    for item in data:
        break_item = re.findall(
            r"^(\w*) would (gain|lose) (\d+) [\w\ ]+ (\w*)\.$", item
        )
        ...
        if (
            relationships.get(name, None) == None
        ):  # ? Could look at module collections with defaultdict
            relationships[name] = {}
...
def work_out_happiness(relationships, relationships_happiness):
    for source_name, target_dict in relationships.items():
        ...
        for target_key, target_val in target_dict.items():
            happiness_match_val = (
                relationships[target_key].get(source_name, 0) + target_val
            )
            relationships_happiness[source_name].update(
                {target_key: happiness_match_val}
            )
    ...
    for seating_ind in range(len(seating_combis)):
        ...
        seating_tmp_score = relationships_happiness[first_position_name][
            last_position_name
        ]

        for pos in range(1, len(seating_combis[seating_ind])):
            seating_tmp_score += relationships_happiness[
                seating_combis[seating_ind][pos]
            ][seating_combis[seating_ind][pos - 1]]
