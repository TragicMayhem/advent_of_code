...
def parse(puzzle_input):
    ...
    for item in data:
        break_item = re.findall(
            r"^(\w*) can fly (\d+) km/s for (\d+) sec.* for (\d+) seconds\.$", item
        )
...
def part1(reindeer, race_time=1000):
    ...
    for k in reindeer.keys():
        winning_dist = (
            reindeer[k]["distance"]
            if reindeer[k]["distance"] > winning_dist
            else winning_dist
        )
...
def part2(reindeer, race_time=1000):
    ...
    for k in reindeer.keys():
        ...
        winning_points = (
            reindeer[k]["points"]
            if reindeer[k]["points"] > winning_points
            else winning_points
        )
        print(
            f"{k} travelled {reindeer[k]['distance']} km in {race_time} seconds. Points {reindeer[k]['points']}"
        )
