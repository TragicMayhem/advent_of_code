...
input = script_path / 'input.txt'  # 821 / 886
input_test = script_path / 'input_test.txt'  # 4 / 17
# Part 2 same test input just lock the corners?
...
def next_state(grid, grid_size, posx, posy):
    ...
    lights_on = sum(filter(None, [grid[x][y] for x,y in get_coords8d(posx, posy, grid_size)]))
...
def change_state(lights_on, size, coords):
    ...
    how_many_on_around = sum(filter(None, [(x,y) in lights_on  for x,y in get_coords8d(x, y, size)]))

    if current_status and 2 <= how_many_on_around <= 3:  # ON already with 2 to 3 around = stays ON
        return True

    if not current_status and how_many_on_around == 3:  # OFF already with exactly 3 on around = turns ON
        return True
