...
stats_me = (100, 0, 0)          # Before buying anything {'Hit Points': 100, 'Damage': 0, 'Armor': 0}
...
def parse():
    ...
    with open(shop_file, 'r') as file:
        ...
        rings_list.append(['Empty', 0, 0, 0, 0])  # Alt add two rings: [0 for i in range(5)] for j in range(2)]
...
def fight(boss, player):
    ...
    while boss_hp > 0 and player_hp > 0:
        ...
        if boss_hp <=0: continue

        player_hp -= max(1, boss_damage - player_armour)  # Always does at least 1 damage
    ...
    for weapon in weapons_list:
        for armour in armour_list:
            for ringL, ringR in combinations(rings_list, 2):
                ...
                player_hp = 100
                player_damage = sum(list(map(int,[weapon[2], armour[2], ringL[2], ringR[2]])))
                player_armour = sum(list(map(int,[weapon[3], armour[3], ringL[3], ringR[3]])))
                print("damage:", player_damage, "armour:", player_armour)
                if fight(boss, (player_hp, player_damage, player_armour)):
                    cost = sum(list(map(int,[weapon[1], armour[1], ringL[1],ringR[1]])))
