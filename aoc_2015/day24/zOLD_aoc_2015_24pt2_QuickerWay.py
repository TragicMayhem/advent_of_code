...
print(
    ".......it is A LOT of combinations and inefficient code.... but it got the right answer"
)
...
def solve_groups_v1(weight_list, num_grps):
    ...
    for g1 in range(4, len(weight_list) // num_grps):
        ...
        for c1 in itertools.combinations(
            weight_list, g1
        ):  # Calculate parcel weights left (not used)  (Possible Group 2 and Group3)
            ...
            if sum(c1) == target_weight:  # Group 1 target weight met
                for g2 in range(4, len(left_after_group1)):
                    for c2 in itertools.combinations(
                        left_after_group1, g2
                    ):  # Calc whats left from previous pool of parcels (Group 3)
                        if (
                            sum(c2) == target_weight
                        ):  # Group 2 combination total equals target weight (and therefore group 3 must too)
                            left_after_group2 = [
                                x for x in left_after_group1 if x not in c2
                            ]  # Calc remaining parcels (would be Group 3)
                            # print("G1 with", c1, "G2 with", c2, " G3 (remaining)", left_after_group2)
                            res.append(
                                c1
                            )  # Orignial was to send back a list of tuples ([c1,c2,tuple(left_after_group2)])
...
def solve_groups_v2(weight_list, num_grps):
    ...
    res = []
    print(
        f"solving for {len(weight_list)} parcel weights and {num_grps} groups and target of {target_weight}"
    )
...
def check_answers(ans):
    ...
    print(
        "Number of combinations with smallest size:",
        len(smallest_g1_combinations),
        "the first few shown below:",
    )
