...
soln_file = script_path / "input.txt"  # 1050 (942 impossible) / 1921
test_file = script_path / "test.txt"  # 5 (4 impossible)
...
def parse(puzzle_input):
    ...
    with open(puzzle_input, "r") as file:
        ...
        data = [
            " ".join(d.split()).split() for d in data
        ]  # Splits/rejoins (to replace the multiple spaces), the splits into list
...
def part1(data):
    ...
    print(
        f"Input: {len(data)} with {len(valid)} valid triangles and {len(impossible)} impossible"
    )
...
def build_tri(g1, g2, g3):
    if not (
        isinstance(g1, list)
        and isinstance(g2, list)
        and isinstance(g3, list)
        and len(g1) == 3
        and len(g2) == 3
        and len(g3) == 3
    ):
