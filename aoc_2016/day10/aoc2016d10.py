    ...
    with open(puzzle_input, 'r') as file:
        ...
        for d in data:
            ...
            if d.startswith('value'):
                ...
                if first_target.startswith('output'): outputs[first_target] = []
                if second_target.startswith('output'): outputs[second_target] = []
...
def part1(data):
    ...
    while queue:
        ...
        if low_target.startswith('out'):
            ...
            if len(bots[low_target]['chips']) == 2: queue.append(low_target)

        if high_target.startswith('out'):
            outputs[high_target].append(values[1])
        else:
            bots[high_target]['chips'].append(values[1])
            if len(bots[high_target]['chips']) == 2: queue.append(high_target)
