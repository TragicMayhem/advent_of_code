...
bag_queue = {search_bag}  # Initialise the queue for later processing.  This will be added to
...
while bag_queue:
  ...
  for k, v in bag_rules.items():
    ...
    if isinstance(v, dict):  # Checks if there is a dictionary for the parent(k) - if so it contains other bags
      if srch in v.keys():  # Check if the current search back is in the keys of the sub dictionary
