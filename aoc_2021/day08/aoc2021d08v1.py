...
def part2(data):
    ...
    for d in data:
        ...
        for g in codes_filter_5char:
            if (
                "3" not in numbers_pattern.keys()
                and containsCount(g, numbers_pattern["1"]) == 2
            ):
                numbers_pattern["3"] = g
                continue

            if (
                "5" not in numbers_pattern.keys()
                and containsCount(g, numbers_pattern["4"]) == 3
            ):
                numbers_pattern["5"] = g
                continue
        ...
        for g in codes_filter_6char:
            ...
            if (
                "9" not in numbers_pattern.keys()
                and containsCount(g, numbers_pattern["4"]) == 4
            ):
                numbers_pattern["9"] = g
                continue

            if (
                "6" not in numbers_pattern.keys()
                and containsCount(g, numbers_pattern["7"]) == 2
            ):
                numbers_pattern["6"] = g
                continue
