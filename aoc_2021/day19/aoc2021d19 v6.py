...
def parse_np(puzzle):
    with open(puzzle, 'r') as file:
        ...
        for scanner_data in scanner_blocks:
            ...
            values = np.array([[int(i) for i in line.split(',')] for line in scanner_data.splitlines()[1:]])
...
def look_for_beacon_match(distances, source, target):
    ...
    for source_idx, sd in enumerate(distances[source]):
        ...
        for target_idx, td in enumerate(distances[target]):
            # print("    td", td)
            matching_beacons_distances = set(sd).intersection(set(td))  # (Think) same as set(sd) & set(td)
...
def workout_all_scanners(scanners, distances):
    ...
    scanner_coordinates[0] = np.array([0,0,0])  # Assumes scanner 0 is at position 0,0,0
    ...
    while tally < len(scanners):
        ...
        for inner_scanner_loop in range(len(scanners)):
            ...
            results = look_for_beacon_match(distances, outer_scanner_count, inner_scanner_loop)
            ...
            for md in any_matches:
                ...
                if md != 0:
                    ...
                    source_diff = scanners[outer_scanner_count][s_idx] - scanners[outer_scanner_count][x]
                    target_diff = scanners[inner_scanner_loop][t_idx] - scanners[inner_scanner_loop][y]
                    ...
                    if len(set(np.abs(source_diff))) < 3: continue
                    ...
                    try:
                        for value in source_diff:
                            find_coord_pos = np.where(np.absolute(target_diff) == abs(value))[0][0]
                            seq.append(find_coord_pos)
                            sign.append(target_diff[find_coord_pos] // value)

                        adjust_scanner = scanners[inner_scanner_loop][:, seq] * np.array(sign)
                        # print(seq, sign)
            ...
            actual_scanner_coordinates = scanners[outer_scanner_count][s_idx] - adjust_scanner[t_idx]
...
def calculate_beacons(scanners):
    ...
    for beacons in scanners:
        ...
        identified_beacons.update(beacons)  # Using sets will force only keeping unique. you Update with a new set of data
