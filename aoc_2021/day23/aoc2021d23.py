...
def move_to_room(all_rooms, hall, room_size):
    ...
    for h_idx, target_location in enumerate(hall):
        ...
        cost = calculate_cost_to_move(
            single_room, hall, target_location, h_idx, room_size, into_room=True
        )
        ...
        changed_rooms = (
            all_rooms[:target_location]
            + ((target_location,) + single_room,)
            + all_rooms[target_location + 1 :]
        )
...
def move_to_hall(all_rooms, hall, room_size):
    ...
    for room_idx, single_room in enumerate(all_rooms):
        ...
        for occupant in single_room:
            if (
                check_in_right_room and occupant == room_idx
        ...
        for h_idx in range(len(hall)):
            cost = calculate_cost_to_move(
                single_room, hall, room_idx, h_idx, room_size, into_room=False
            )
            ...
            changed_room = (
                all_rooms[:room_idx] + (single_room[1:],) + all_rooms[room_idx + 1 :]
...
def calculate_cost_to_move(room, hall, room_idx, hall_idx, room_size, into_room=False):
    ...
    return 10**pod * (
        DISTANCES_CALCULATED[room_idx][hall_idx] + (into_room + room_size - len(room))
    )
