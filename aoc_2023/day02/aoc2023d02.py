...
def parse(puzzle_input):
    ...
    for game_info in game_records:
        game_id = game_info[0]
        draws = [tuple(p.strip() for p in d.split(',')) for d in game_info[1].split(";")]
    ...
    for k,v in data.items():
        ...
        for draws in v:
            for draw in draws:
                ...
                # print(count, colour)
                count = int(count)  
                cube_max[colour] = {True: count, False: cube_max.get(colour, 0)} [cube_max.get(colour, 0) < count]
                
        game_score = cube_max.get('red',0 ) * cube_max.get('green',0 ) * cube_max.get('blue',0 )
