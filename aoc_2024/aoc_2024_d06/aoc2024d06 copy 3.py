...
def check_for_next_obstacle(h, w, obs, current_pos, dn):
    ...
    locked_dimension = 1 if dx == 0 else 0
    ...
    if locked_dimension == 1:
        filtered_tuples = [
            tup
            for tup in obs
            if tup[locked_dimension] == target
            # and (tup[1 - locked_dimension] - moveable) * step_ns < 0
        ]
        filtered_tuples = sorted(filtered_tuples, key=lambda x: abs(x[1] - moveable))
    else:
        ...
        filtered_tuples = sorted(filtered_tuples, key=lambda x: abs(x[0] - moveable))

    if filtered_tuples:
        walking_to_pos = filtered_tuples[0]
    else:
        # Handle edge cases
        if dy == -1:
            walking_to_pos = (0, c) if dx == 0 else (r, 0)
        else:
            walking_to_pos = (h - 1, c) if dx == 0 else (r, w - 1)
